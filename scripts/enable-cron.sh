...
CRON_LINE="${CRON_SCHEDULE} ${DOCKER_BIN} exec -u root ${CONTAINER} pip3 install -U --pre --no-cache-dir --break-system-packages 'yt-dlp[default,curl-cffi]' >> ${CRON_LOG} 2>&1 ${MARKER}"
