...
#### Підтримання `yt-dlp` в актуальному стані

`yt-dlp` часто ламається — сайти змінюють свої плеєри, і екстрактори потребують частих оновлень, тому застарілий `yt-dlp` рано чи пізно почне завалювати завантаження (це і є сигнал оновитися). Оновлюй його **лише через pip** (ніколи не `yt-dlp -U` для pip-встановлення).

Docker-образ фіксує останню версію на момент збірки. Щоб оновити пізніше, запусти `pip install -U` **всередині працюючого контейнера** — без рестарту, без даунтайму, і бот підхопить новий `yt-dlp` на наступному завантаженні:

```bash
make update-ytdlp          # docker exec <container> pip install -U yt-dlp
```

Щоб робити це автоматично, постав на хост щотижневий крон:

```bash
make enable-cron           # додає в crontab щотижневий `docker exec ... pip install -U yt-dlp` (див. scripts/enable-cron.sh)
```

Типово: понеділок 04:00, контейнер `video-downloader-bot`, лог `/var/log/crons/video-downloader-bot-ytdlp-weekly-update.log` (перевизначається через `CONTAINER` / `CRON_SCHEDULE` / `CRON_LOG`). Повторний запуск безпечний — запис замінюється, а не дублюється.

Як варіант, контейнер може сам оновлювати `yt-dlp` **на старті** — це опційно і типово вимкнено, щоб звичайні рестарти були швидкими. Увімкнути для конкретного запуску:

```bash
docker run -e YTDLP_SELFUPDATE=true ... video-downloader-bot
```

> Оновлення best-effort: якщо pip не достукається до PyPI (наприклад, через DPI), `yt-dlp` залишиться на поточній версії.
